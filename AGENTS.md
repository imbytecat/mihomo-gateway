...
NixOS VM 透明代理网关（Mihomo + nftables TPROXY）。产物是 qcow2 镜像。
...
just check    # nix flake check（等同构建 toplevel，本仓库唯一"测试"）
just fmt      # nixfmt
...
- **单臂代理**：只拦截 transit（forward）流量，不代理本机 OUTPUT。**不要加 OUTPUT 链**。
...
- **rp_filter 必须通过 networkd 逐接口禁用**（`ens*` + `lo` 都要）。sysctl `all`/`default` 不足以覆盖 NixOS 默认值 2。
